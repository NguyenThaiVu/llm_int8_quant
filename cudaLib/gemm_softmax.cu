...
__global__ void softmax_lastdim_int8_kernel(
    ...
    const float* __restrict__ scale_x,  // length num_vecs
    const float* __restrict__ scale_y,  // length num_vecs
    int64_t num_vecs,
    int64_t dim2)
{
    ...
    float warp_m = warp_reduce_max(local_max);
    ...
    float warp_s = warp_reduce_sum(local_sum);
...
torch::Tensor softmax_lastdim_int8_cuda(
    ...
    softmax_lastdim_int8_kernel<<<grid, block, shared_mem_size, stream>>>(
            xq.data_ptr<int8_t>(),
            y_q.data_ptr<int8_t>(),
            sx.data_ptr<float>(),
