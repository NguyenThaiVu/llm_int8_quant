...
// ================================================================
/*
RMSNorm kernel for BF16 input and BF16 output

Input:
    x     : BF16, shape (..., d_model)
    gamma : FLOAT32 or BF16, shape (d_model)

Output:
    y     : BF16, same shape as x
*/
// ================================================================

template <typename GammaT>
__global__ void rmsnorm_bf16_kernel(
    const at::BFloat16* __restrict__ x,      // [num_rows, d_model]
    const GammaT* __restrict__ gamma,        // [d_model]
    at::BFloat16* __restrict__ y,            // [num_rows, d_model]
    int d_model,
    float eps
) {
    ...
    const at::BFloat16* row_x = x + static_cast<int64_t>(row) * d_model;
    at::BFloat16* row_y = y + static_cast<int64_t>(row) * d_model;
    ...
    for (int col = tid; col < d_model; col += blockDim.x) {
        float x_fp = static_cast<float>(row_x[col]);
    ...
    // ------------------------------------------------------------
    // Pass 2: Apply RMSNorm and gamma, then cast to BF16
    // ------------------------------------------------------------
    for (int col = tid; col < d_model; col += blockDim.x) {
        float x_fp = static_cast<float>(row_x[col]);
        float gamma_val = static_cast<float>(gamma[col]);

        float y_fp = x_fp * rms_inv * gamma_val;

        row_y[col] = static_cast<at::BFloat16>(y_fp);
    }
}

torch::Tensor rmsnorm_bf16_cuda(
    torch::Tensor x,       // BF16 - (..., d_model)
    torch::Tensor gamma,   // FLOAT32 or BF16 - (d_model)
    float eps
) {
    ...
    AT_DISPATCH_SWITCH(
        ...
        AT_DISPATCH_CASE(at::ScalarType::Float, [&] {
            rmsnorm_bf16_kernel<float><<<grid, block, 0, stream>>>(
        ...
        AT_DISPATCH_CASE(at::ScalarType::BFloat16, [&] {
            rmsnorm_bf16_kernel<at::BFloat16><<<grid, block, 0, stream>>>(
                x_contig.data_ptr<at::BFloat16>(),
...
// ================================================================
/*
RMSNorm kernel for BF16 input and INT8 output with SmoothQuant scale

Input:
    x            : BF16, shape (..., d_model)
    gamma        : FLOAT32 or BF16, shape (d_model)
    smooth_scale : FLOAT32, shape (d_model)

Output:
    y       : INT8, same shape as x
    scale_y : FLOAT32, shape (...), per-row output scale
*/
// ================================================================

template <typename GammaT>
__global__ void rmsnorm_bf16_to_int8_kernel(
    const at::BFloat16* __restrict__ x,          // [num_rows, d_model]
    const GammaT* __restrict__ gamma,            // [d_model]
    const float* __restrict__ smooth_scale,      // [d_model]
    int8_t* __restrict__ y,                      // [num_rows, d_model]
    float* __restrict__ scale_y,                 // [num_rows]
    int d_model,
    float eps
) {
    const int row = blockIdx.x;
    ...
    const at::BFloat16* row_x = x + static_cast<int64_t>(row) * d_model;
    int8_t* row_y = y + static_cast<int64_t>(row) * d_model;
    ...
    // ------------------------------------------------------------
    // Pass 2: Compute max absolute smoothed RMSNorm output
    //
    // Important:
    // amax must be computed AFTER dividing by smooth_scale[col],
    // because this is the actual value we will quantize.
    // ------------------------------------------------------------
    ...
    for (int col = tid; col < d_model; col += blockDim.x) {
        float x_fp = static_cast<float>(row_x[col]);
        float gamma_val = static_cast<float>(gamma[col]);
        float smooth_val = smooth_scale[col];

        float y_fp = x_fp * rms_inv * gamma_val / smooth_val;

        local_max_abs = fmaxf(local_max_abs, fabsf(y_fp));
    }
    ...
    if (tid == 0) {
        ...
        // All-zero row: avoid divide-by-zero.
        // Quantized output will still be all zeros.
        if (s == 0.0f) {
            s = 1.0f;
        }
    ...
    // ------------------------------------------------------------
    // Pass 3: Quantize smoothed RMSNorm output to INT8
    // ------------------------------------------------------------
    for (int col = tid; col < d_model; col += blockDim.x) {
        float x_fp = static_cast<float>(row_x[col]);
        float gamma_val = static_cast<float>(gamma[col]);
...
std::tuple<torch::Tensor, torch::Tensor> rmsnorm_bf16_to_int8_cuda(
    torch::Tensor x,             // BF16 - (..., d_model)
    torch::Tensor gamma,         // FLOAT32 or BF16 - (d_model)
    torch::Tensor smooth_scale,  // FLOAT32 - (d_model)
    float eps
) {
    TORCH_CHECK(x.is_cuda(), "x must be CUDA");
    TORCH_CHECK(x.scalar_type() == torch::kBFloat16, "x must be bfloat16");
    ...
    TORCH_CHECK(x.numel() > 0, "x must be non-empty");
    TORCH_CHECK(x.size(-1) == gamma.size(0), "gamma size must match x.size(-1)");
    TORCH_CHECK(x.size(-1) == smooth_scale.size(0), "smooth_scale size must match x.size(-1)");
    ...
    AT_DISPATCH_SWITCH(
        gamma_contig.scalar_type(),
        "rmsnorm_bf16_to_int8_cuda",
        AT_DISPATCH_CASE(at::ScalarType::Float, [&] {
            rmsnorm_bf16_to_int8_kernel<float><<<grid, block, 0, stream>>>(
                x_contig.data_ptr<at::BFloat16>(),
                gamma_contig.data_ptr<float>(),
        ...
        AT_DISPATCH_CASE(at::ScalarType::BFloat16, [&] {
            rmsnorm_bf16_to_int8_kernel<at::BFloat16><<<grid, block, 0, stream>>>(
                x_contig.data_ptr<at::BFloat16>(),
                gamma_contig.data_ptr<at::BFloat16>(),
                smooth_scale_contig.data_ptr<float>(),
                y.data_ptr<int8_t>(),
...
- Input: x -  INT8, shape (..., d_model) 
    ...
    smooth_scale: FLOAT32, shape (d_model) - the "smooth quantization" scale for each dimension
- Output: 
