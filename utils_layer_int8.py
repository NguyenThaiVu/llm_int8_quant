...
class Custom_Linear(nn.Module):
    ...
    def forward(self, x, scale_x):
        if not self.is_quantized:  # Calibration mode 
            ...
            print(f"scale_y: {self.scale_y.shape}")
...
class Custom_Element_Wise(torch.nn.Module):
    ...
    def forward(self, a, scale_a, b, scale_b):
        if self.is_quantized:
            out_int8 = gemm_cutlass.func_element_wise_mul_int8(a, scale_a.to(torch.float32),\
                                b, scale_b.to(torch.float32),\
                                self.scale_out.to(torch.float32))
...
class Custom_FeedForward(nn.Module):
    def __init__(self, cfg):
        ...
        self.custom_elementwise_mul = Custom_Element_Wise()
