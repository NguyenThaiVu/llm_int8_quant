...
def load_weights_into_qwen(model, param_config, params):
    ...
    for l in range(param_config["n_layers"]):
        ...
        if hasattr(att, "q_norm") and att.q_norm is not None:
            if hasattr(att.q_norm, "scale"):
                att.q_norm.scale = assign(
                    att.q_norm.scale,
                    params[f"model.layers.{l}.self_attn.q_norm.weight"],
                    f"model.layers.{l}.self_attn.q_norm.weight"
                )
            elif hasattr(att.q_norm, "weight"):
                ...
                raise ValueError(f"Layer {l} block has no recognizable q_norm parameters.")
        if hasattr(att, "k_norm") and att.k_norm is not None:
            if hasattr(att.k_norm, "scale"):
                att.k_norm.scale = assign(
                    att.k_norm.scale,
                    params[f"model.layers.{l}.self_attn.k_norm.weight"],
                    f"model.layers.{l}.self_attn.k_norm.weight"
                )
            elif hasattr(att.k_norm, "weight"):
                ...
                raise ValueError(f"Layer {l} block has no recognizable k_norm parameters.")

        # Attention layernorm
        if hasattr(block.norm1, "scale"):  
            block.norm1.scale = assign(
                block.norm1.scale,
                params[f"model.layers.{l}.input_layernorm.weight"],
                f"model.layers.{l}.input_layernorm.weight"
            )
        elif hasattr(block.norm1, "weight"):
            ...
            raise ValueError(f"Layer {l} block has no recognizable norm1 parameters.")
        ...
        if hasattr(block.norm2, "scale"):
            block.norm2.scale = assign(
                block.norm2.scale,
                params[f"model.layers.{l}.post_attention_layernorm.weight"],
                f"model.layers.{l}.post_attention_layernorm.weight"
            )
        elif hasattr(block.norm2, "weight"):
            ...
            raise ValueError(f"Layer {l} block has no recognizable norm2 parameters.")
